...
iant data for genetic association, typically in case-control studies.
 License: GPL-3
 Depends: survival
 ...
 * phi_mlR
 *
 * MLE of phi (over-dispersion parameter), given mu to be accesed from R
 *
 * Actually we find MLE of 1/phi here and then take inverse
 *
 **********************************************************************/

void phi_mlR(double* y, double* mu, int *RN, int *Rlimit, double *Reps, 
           double* phi, int *RinitPhi, int *Rtrace, double *prior)
{
  double theta0, del, tmp;
  double score=0.0;
  double info=0.0;
                   ...
                   double* y, double* x, double* mu, double* mu1)
{
  ...
  double prior[N];
  for(i=0;i<N;i++) prior[i]=1.0;
    logL = loglik_NB(N, phi, mu1, y, prior);
  }else {
    error("wrong family\n");
  }
             ...
             double *fitted2, double *offsetN)
{
  ...
  if (*family==NB) {
    twoLL_trec0 = 2.0*loglik_NB(N, phi_old, fitted2, pY, prior);
  ...
  double prior[N];
  for(i=0;i<N;i++) prior[i]=1.0;
      twoLL_trec1 = 2.0*loglik_NB(N, phi_old, fitted2, pY, prior);
    }else if (*family==POISSON) {
...
/**********************************************************************
 *
 * Forward backward algorithm for two state hmm
 * 
 **********************************************************************/

void forwardback(double *Pi, double *pi, double *prob, int *Rn, int *Rm, double *logalpha, double *logbeta, double *LL){
	//Pi is a 4x1 vector, column stacked transition prob matrix on log scale
	//pi is initial probability state vector on log scale
	//prob is the column stacked emission prob matrix on log scale
	//n is the number of obs
	//m is the number of states
	int n = *Rn;
	int m = *Rm;
	int i;
	double phi[m], lscale, sumphi, phi0old, phi1old, logsumtemp[2];	
	...
	*LL = 0;
	...
	for(i=0; i<n; i++){
		...
		if(i==0){
			phi[0] = phi[0] + prob[i];
			phi[1] = phi[1] + prob[n+i];
		}else{
			phi0old = phi[0];
			phi1old = phi[1];
			//phi[0] = (phi0old*Pi[0] + phi1old*Pi[1])*prob[i];
			//phi[1] = (phi0old*Pi[2] + phi1old*Pi[3])*prob[n+i];
	
			//phi is not not on log scale at this point
			logsumtemp[0] = phi0old+Pi[0]+prob[i];
			logsumtemp[1] = phi1old+Pi[1]+prob[i];
			phi[0] =logsumexp(logsumtemp, 0, 1);

			logsumtemp[0] = phi0old+Pi[2]+prob[i+n];
			logsumtemp[1] = phi1old+Pi[3]+prob[i+n];
			phi[1] =logsumexp(logsumtemp, 0, 1);
			//phi[1] = (phi0old*Pi[2] + phi1old*Pi[3])*prob[n+i];
		}
		/*sumphi = phi[0]+phi[1];
		phi[0] = phi[0]/sumphi;
		...
		logalpha[i] = log(phi[0]) + lscale;
		logalpha[n+i] = log(phi[1]) + lscale;*/
		sumphi = logsumexp(phi, 0, 1);
		phi[0] = phi[0]- sumphi;
		phi[1] = phi[1]- sumphi;
		lscale = lscale + sumphi;
		logalpha[i] = phi[0] + lscale;
		logalpha[n+i] = phi[1]+ lscale;
	}
	*LL = lscale;

	//backward prob
	phi[0] = phi[1] = log(1.0/m);
	lscale = log(m);
	for(i=n-2; i>=0; i--){
		phi0old = phi[0];
		phi1old = phi[1];
		
		logsumtemp[0] = phi0old+Pi[0]+prob[i+1];
		logsumtemp[1] = phi1old+Pi[2]+prob[i+n+1];
		phi[0] =logsumexp(logsumtemp, 0, 1);

		logsumtemp[0] = phi0old+Pi[1]+prob[i+1];
		logsumtemp[1] = phi1old+Pi[3]+prob[i+n+1];
		phi[1] =logsumexp(logsumtemp, 0, 1);
		//phi[0] = (phi0old*Pi[0] + phi1old*Pi[2])*prob[i+1];
		//phi[1] = (phi0old*Pi[1] + phi1old*Pi[3])*prob[n+i+1];
		
		logbeta[i] = phi[0] + lscale;
		logbeta[n+i] = phi[1] + lscale;
		sumphi = logsumexp(phi, 0, 1);
		phi[0] = phi[0]- sumphi;
		phi[1] = phi[1]- sumphi;
		lscale = lscale + sumphi;
...
void forwardbackaug(double *Pi, double *pi, double *prob, int *Rn, int *Rm, double *logalpha, double *logbeta, double *LL){
	//Pi is a 4x1 vector, column stacked transition prob matrix on log scale
	//pi is initial probability state vector on log scale
	//prob is the column stacked emission prob matrix on log scale (m^2 columns)
	//n is the number of obs
	//m is the number of states
	int n = *Rn;
	int m = *Rm;
	int i;
	double phi[m], lscale, sumphi, phi0old, phi1old, logsumtemp[m];	
	...
	*LL = 0;
	...
	for(i=0; i<n; i++){
		...
		if(i==0){
			phi[0] = phi[0] + prob[i];
			phi[1] = phi[1] + prob[2*n+i]; //first obs has same prob in columns 1 and 2, 3, and 4
			//Rprintf("%f %f %f %f %f %f\n", prob[i], prob[i+n], prob[i+2*n], prob[i+3*n], phi[0], phi[1]);
		}else{
			phi0old = phi[0];
			phi1old = phi[1];
			//phi[0] = (phi0old*Pi[0] + phi1old*Pi[1])*prob[i];
			...
			//phi is not not on log scale at this point
			logsumtemp[0] = phi0old+Pi[0]+prob[i];
			logsumtemp[1] = phi1old+Pi[1]+prob[i+n];
			phi[0] =logsumexp(logsumtemp, 0, 1);

			logsumtemp[0] = phi0old+Pi[2]+prob[i+2*n];
			logsumtemp[1] = phi1old+Pi[3]+prob[i+3*n];
			phi[1] =logsumexp(logsumtemp, 0, 1);
			//phi[1] = (phi0old*Pi[2] + phi1old*Pi[3])*prob[n+i];
		}
		/*sumphi = phi[0]+phi[1];
	...
	*LL = lscale;
	...
	for(i=n-2; i>=0; i--){
		...
		logsumtemp[0] = phi0old+Pi[0]+prob[i+1];
		logsumtemp[1] = phi1old+Pi[2]+prob[i+2*n+1];
		phi[0] =logsumexp(logsumtemp, 0, 1);

		logsumtemp[0] = phi0old+Pi[1]+prob[i+n+1];
		logsumtemp[1] = phi1old+Pi[3]+prob[i+3*n+1];
		phi[1] =logsumexp(logsumtemp, 0, 1);
		//phi[0] = (phi0old*Pi[0] + phi1old*Pi[2])*prob[i+1];
		//phi[1] = (phi0old*Pi[1] + phi1old*Pi[3])*prob[n+i+1];
