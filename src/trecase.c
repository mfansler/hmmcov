...
void grad_b(double b1, int N, int h, double b0, double phi,
            double theta, double* y, double*x, double* mu, 
            double* nA, double* nTotal, double* zeta, double* gr) 
{
  int i, k;
  ...
  // printf("nTotal[h-1]=%e, nA[h-1]=%e, zeta[h-1]=%e\n", nTotal[h-1], nA[h-1], zeta[h-1]);
  ...
  for (i=0; i<N; i++) {
    ...
    if (mui < 1e-8) {
      warning("invalid mui: %e @ grad_b, set to 1e-8\n", mui);
      mui = 1e-8;
    }
          ...
          int limit, double eps, int trace, int* failR)
{
  ...
  while (it < limit && fabs(del) > eps) {
    ...
    grad_b(b1, N, h, b0, phi, theta, y, x, mu, nA, nTotal, zeta, gr);
  ...
  for(i=0; i<nY; i++,pY+=N,pY1+=N,pY2+=N){
    ...
    if(useASE){
      ...
      if (fail) {
        ...
        if(*trace > 1){
          negGradLogH0(npara, initPara, gr, (void*)exPara);
    ...
    for(j=0; j<nZ; j++,pZ+=N,pZh+=N){
	  ...
	  double prior[N];
          for(i=0;i<N;i++) prior[i]=1.0;
          twoLL_trec_joint0 = 2.0*loglik_NB(N, phi_old, fitted2, pY, prior);
        ...
        twoLL_ase_joint0 = - 2.0*negLogH1(2, para_ase, exPara);
	    ...
	    double prior[N];
	    for(i=0;i<N;i++) prior[i]=1.0;
            twoLL_trec_joint1 = 2.0*loglik_NB(N, phi_old, fitted2, pY,prior);
          }else if (family==POISSON) {
          ...
          twoLL_ase_joint1 = - 2.0*negLogH1(2, para_ase, exPara);
